...
unsigned char *fast_rescaleImage(unsigned char *src, int src_x, int src_y, int dest_x, int dest_y)
{
 ...
 unsigned char R1,R2,R3,R4;   // Colours at the four neighbours //turn these into arrays
 unsigned char G1,G2,G3,G4;
 unsigned char B1,B2,B3,B4;

 double RT1, GT1, BT1;      // Interpolated colours at T1 and T2 //potentially these aswell
 double RT2, GT2, BT2;
 ...
 for (y=0;y<dest_y;y++){
  fy=y*step_y;
  fly=(int)fy;                                                  //               |
  cly=(fly==fy)? fly: fly+1;
  dy=fy-fly;
  for (x=0;x<dest_x;x++)     // Loop over destination image
  {
   fx=x*step_x;
   flx=(int)fx;                                                  //Extrapolate this into one function  ;
   clx=(flx==fx)? flx: flx+1;
   dx=fx-flx;
                                                  //               |
                                                                   //               |   
   // getPixel(src,flx,fly,src_x,&R1,&G1,&B1); // get N1 colours
   R1=*(src+((flx+(fly*src_x))*3)+0);
   G1=*(src+((flx+(fly*src_x))*3)+1);
   B1=*(src+((flx+(fly*src_x))*3)+2);      //               |    
   // getPixel(src,clx,fly,src_x,&R2,&G2,&B2);  // get N2 colours     //               |
   R2=*(src+((clx+(fly*src_x))*3)+0);
   G2=*(src+((clx+(fly*src_x))*3)+1);
   B2=*(src+((clx+(fly*src_x))*3)+2);
   // getPixel(src,flx,cly,src_x,&R3,&G3,&B3);  // get N3 colours     //               |
   R3=*(src+((flx+(cly*src_x))*3)+0);
   G3=*(src+((flx+(cly*src_x))*3)+1);
   B3=*(src+((flx+(cly*src_x))*3)+2); 
   // getPixel(src,clx,cly,src_x,&R4,&G4,&B4); // get N4 colours      //               |
   R4=*(src+((clx+(cly*src_x))*3)+0);
   G4=*(src+((clx+(cly*src_x))*3)+1);
   B4=*(src+((clx+(cly*src_x))*3)+2);
